# Agent Guide — ep_sticky_attributes
...
`ep_sticky_attributes` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_sticky_attributes`:
...
* Client: `postAceInit`, `aceEditEvent`, `aceKeyEvent`, `aceAttribsToClasses`, `aceEditorCSS`
