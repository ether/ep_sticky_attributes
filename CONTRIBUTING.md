# Contributing to ep_sticky_attributes

Thanks for helping improve `ep_sticky_attributes` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_sticky_attributes` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_sticky_attributes from your local clone
pnpm run plugins i --path ../ep_sticky_attributes

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_sticky_attributes/` are picked up after a server restart.
...
* Etherpad version, ep_sticky_attributes version, browser + OS
* Steps to reproduce
